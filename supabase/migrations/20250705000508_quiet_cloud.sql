/*
  # Complete Travel Database Schema with Travel Buddies & Itineraries

  1. New Tables
    - `users` - Enhanced user profiles with travel preferences
    - `travel_preferences` - Detailed travel preferences and requirements
    - `trips` - AI-generated itineraries with JSONB storage
    - `itineraries` - Normalized daily travel plans
    - `matches` - Travel buddy matching system with compatibility scores
    - `buddy_requests` - Direct connection requests between travelers
    - `trip_participants` - Group travel management
    - `saved_itineraries` - Bookmarked trips for inspiration

  2. Security
    - Enable RLS on all tables
    - Add policies for authenticated users to manage their own data
    - Allow reading public profiles and trips

  3. Performance
    - Add indexes for common queries
    - Optimize buddy finding and matching operations

  4. Business Logic
    - Functions for calculating compatibility scores
    - Automatic trip duration calculation
    - Travel buddy discovery algorithm
*/
...
CREATE POLICY "Users can manage own itineraries" ON itineraries
  FOR ALL TO authenticated
  USING (trip_id IN (
    SELECT id FROM trips WHERE user_id = (SELECT id FROM users WHERE email = auth.email())
  ));
...
CREATE POLICY "Users can view own matches" ON matches
  FOR SELECT TO authenticated
  USING (
    user_id_1 = (SELECT id FROM users WHERE email = auth.email()) OR
    user_id_2 = (SELECT id FROM users WHERE email = auth.email())
  );
...
CREATE POLICY "Users can view buddy requests" ON buddy_requests
  FOR SELECT TO authenticated
  USING (
    from_user_id = (SELECT id FROM users WHERE email = auth.email()) OR
    to_user_id = (SELECT id FROM users WHERE email = auth.email())
  );

CREATE POLICY "Users can send buddy requests" ON buddy_requests
  FOR INSERT TO authenticated
  WITH CHECK (from_user_id = (SELECT id FROM users WHERE email = auth.email()));

CREATE POLICY "Users can respond to buddy requests" ON buddy_requests
  FOR UPDATE TO authenticated
  USING (to_user_id = (SELECT id FROM users WHERE email = auth.email()));
