...
DROP FUNCTION IF EXISTS record_swipe(uuid, uuid, boolean);
DROP FUNCTION IF EXISTS get_potential_travel_buddies(uuid);
...
-- Function to record a swipe and create a match if mutual
CREATE FUNCTION record_swipe(swiper_user_id uuid, swiped_user_id uuid, is_liked boolean)
...
-- Function to get potential travel buddies
CREATE FUNCTION get_potential_travel_buddies(user_id uuid)
...
CREATE FUNCTION get_user_matches(user_id uuid)
RETURNS TABLE(id uuid, username text, avatar_url text, bio text)
LANGUAGE sql AS $$
