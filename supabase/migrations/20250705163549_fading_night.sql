/*
  # Fix travel buddy notifications

  1. New Tables
    - No new tables created
  2. Security
    - Maintain RLS policies for notifications
    - Ensure proper function creation with IF NOT EXISTS checks
  3. Changes
    - Fix function creation to avoid conflicts
    - Ensure proper dependency handling for triggers and functions
*/
...
BEGIN
  ...
  IF EXISTS (
    ...
    DROP TRIGGER create_notifications_on_match ON travel_matches;
  END IF;
  
  -- Now drop function if it exists
  IF EXISTS (
    SELECT 1 FROM information_schema.routines 
    WHERE routine_name = 'create_match_notifications'
  ) THEN
    DROP FUNCTION create_match_notifications();
  END IF;
END $$;
...
CREATE FUNCTION mark_notifications_as_read(notification_ids uuid[])
...
CREATE FUNCTION get_unread_notification_count(user_id uuid)
...
CREATE FUNCTION get_recent_notifications(user_id uuid, limit_count integer DEFAULT 10)
RETURNS TABLE(
  id uuid,
